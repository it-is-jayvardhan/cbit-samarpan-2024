...
export default function CulturalScreening(){
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            await doc.loadInfo();
            const firstSheet=doc.sheetsByIndex[10];
            ...
            const mappedData = rows.map(row => ({
                ...
                name: row._rawData[1],
                sem: row._rawData[8],
                branch: row._rawData[7],                
                usn: row._rawData[2],
                contactNumber:row._rawData[3],
                event:row._rawData[4],
                agree:row._rawData[5],
                email:row._rawData[6],
              }));
...
//   WebkitTextStroke: '1px black'
            ...
            Cultural & Screening Participants
            </Typography>
        ...
        <Table className={classes.table} aria-label="customized table">
          <TableHead>
            <TableRow className='header'>
              ...
              <TableCell className={classes.header}>Performance</TableCell>
